...
HEADERS = ['Census_Release_Number', 'Dataset_Mnemonic', 'Id']

REQUIRED_FIELDS = {'Dataset_Mnemonic': 'DS1',
                   'Census_Release_Number': 'CR1',
                   'Id': '1'}

INPUT_DIR = os.path.join(pathlib.Path(__file__).parent.resolve(), 'testdata')

FILENAME = os.path.join(INPUT_DIR, 'Release_Dataset.csv')

class TestReleaseDataset(unittest.TestCase):
    def run_test(self, rows, expected_error):
        with unittest.mock.patch('builtins.open', conditional_mock_open('Release_Dataset.csv',
                read_data = build_test_file(HEADERS, rows))):
            with self.assertRaisesRegex(ValueError, expected_error):
                Loader(INPUT_DIR, None).load_dataset_to_releases(['DS1', 'DS2', 'DS3'])
    ...
    def test_invalid_values(self):
        for field in ['Census_Release_Number', 'Dataset_Mnemonic']:
            with self.subTest(field=field):
                row = REQUIRED_FIELDS.copy()
                row[field] = 'X'
    ...
    def test_duplicate_entry(self):
        self.run_test(
            ...
            f'^Reading {FILENAME}:3 duplicate value combo DS1/CR1 for Dataset_Mnemonic/Census_Release_Number$')
